...
Запустил контейнер с явным указанием начальной точки `docker run -it --entrypoint sh ket9/otus-devsecops-owasp-rest:latest`.
...
После докерфайла собрал новый образ `docker build -t secure-owasp-rest .`. Запустил контейнер `docker run --rm -it --entrypoint sh secure-owasp-rest`. 
...
# 🌐 Часть 3
#### Условие - "Прогнать образ через один из сканеров безопасности:
